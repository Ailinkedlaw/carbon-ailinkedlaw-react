  ...
  useEffect(function () {
    if (!(previousState !== null && previousState !== void 0 && previousState.open) && open) {
      ...
      if (initialStep && totalSteps && Number(initialStep) > Number(totalSteps) || Number(initialStep) <= 0) {
        ...
        "".concat(componentName, ": An invalid `initialStep` prop was supplied. The `initialStep` prop should be a number that is greater than 0 or less than or equal to the number of steps your ").concat(componentName, " has."));
      }
    }
  }, [firstIncludedStep, open, previousState, setCurrentStep, initialStep, totalSteps, componentName]);
};

export { useResetCreateComponent };
