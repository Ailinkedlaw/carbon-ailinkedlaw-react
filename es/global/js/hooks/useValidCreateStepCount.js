...
var useValidCreateStepCount = function useValidCreateStepCount(stepCount, componentName) {
  useEffect(function () {
    if (stepCount === 1) {
      ...
      "".concat(componentName, ": ").concat(componentName, "s with one step are not permitted. If you require only one step, please use either the CreateTearsheetNarrow, CreateSidePanel, or CreateModal components."));
    }
  }, [stepCount, componentName]);
};

export { useValidCreateStepCount };
